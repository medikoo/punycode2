...
var QUnit = (global.QUnit = require("qunitjs/qunit/qunit.js"));
delete global.addEventListener;

var qe = require("qunit-extras/qunit-extras.js");
if (qe) {
...
var allSymbols = require("./data.js");
var testData = {
	strings: [
		{ description: "a single basic code point", decoded: "Bach", encoded: "Bach-" },
		{ description: "a single non-ASCII character", decoded: "\xFC", encoded: "tda" },
		{
	...
	ucs2: [
		...
		{
			description:
				"Unmatched high surrogate, followed by a surrogate pair, followed by an" +
		...
		{
			description:
				"Unmatched low surrogate, followed by a surrogate pair, followed by an" +
	...
	domains: [
		...
		{ description: "Emoji", decoded: "\uD83D\uDCA9.la", encoded: "xn--ls8h.la" },
		{
...
test("punycode.ucs2.decode", function () {
	...
	each(allSymbols, function (string, codePoint) {
		deepEqual(
			punycode.ucs2.decode(string), [codePoint],
			"Decoding symbol with code point " + codePoint
		);
	});
	...
	raises(
		function () { punycode.decode("\x81-"); }, RangeError,
...
test("punycode.toUnicode", function () {
	...
	each(testData.strings, function (object) {
		var message =
			"Domain names (or other strings) that don't start with " +
			"`xn--` may not be converted";
...
test("punycode.toASCII", function () {
	...
	each(testData.strings, function (object) {
		equal(
			punycode.toASCII(object.encoded), object.encoded,
